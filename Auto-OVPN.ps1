...
md $TempDir -Force
Invoke-WebRequest -Uri $OVPN24 -OutFile "$TempDir\OVPN.exe"
...
Start-Process -FilePath $TempDir\OVPN.exe -ArgumentList /S -Wait
$ovpndir=(New-Object -ComObject WScript.Shell).RegRead("HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\OpenVPN\InstallLocation") | %{$_.Substring(0, $_.length - 1) }
$powerrsadir="$ovpndir\powerrsa"
md $powerrsadir -Force
Copy-Item -Path $TempDir\Power-RSA-master\* -Destination $powerrsadir -Recurse
...
Add-Content $ServerConfigFile "proto udp" -Force -ErrorAction SilentlyContinue
...
Add-Content $ServerConfigFile "cipher AES-256-CBC" -Force -ErrorAction SilentlyContinue
...
Add-Content $ServerConfigFile "status openvpn-status.log" -Force -ErrorAction SilentlyContinue
...
Add-Content -path "$ServerConfigFile" "<tls-auth>"
Get-Content -path "$env:KEY_DIR\ta.key" | Add-Content -path "$ServerConfigFile"
Add-Content -path "$ServerConfigFile" "</tls-auth>"
cls
...
Add-Content "$ClientConfig" "proto udp" -Force -ErrorAction SilentlyContinue
...
Add-Content "$ClientConfig" "cipher AES-256-CBC" -Force -ErrorAction SilentlyContinue
...
Add-Content -path "$ClientConfig" "<tls-auth>"
Get-Content -path "$ovpndir\powerrsa\pki\ta.key" | Add-Content -path "$ClientConfig"
Add-Content -path "$ClientConfig" "</tls-auth>"
...
Set-Service -Name OpenVPNServiceLegacy -StartupType Disabled
Stop-Service -Name OpenVPNServiceLegacy
...
Start-Process "$env:ovpndir\Uninstall.exe" -ArgumentList "/S" -Wait
